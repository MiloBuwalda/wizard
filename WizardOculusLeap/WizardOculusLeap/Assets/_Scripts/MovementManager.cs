...
//		}
    	...
    	if (hands.Length > 1)
    	{
			...
			if (Vector3.Dot(directionRight, normalRight) > directionRight.sqrMagnitude * 0.5f && Vector3.Dot(direction1, normal1) > direction1.sqrMagnitude * 0.5f && magicBallFase == 1)
      		{
				//Rigidbody magicBallClone = (Rigidbody)Instantiate (magicBall, leapMotionOVRController.transform.position, cameraDirection.transform.rotation);
				//magicBallClone.velocity = cameraDirection.transform.forward * speed;
				magicBallFase = 0;
      		}
			else if (magicBallFase == 0)
			{
				magicBallFase = 1;
			}
    	}
