...
public class Earthscript2 : MonoBehaviour {
    ...
    void Start()
    {
		...
		Color c0 = new Color ();
		float r = Random.Range(55.0f,212.0f);
		float g = Random.Range(55.0f,212.0f);
		float b = Random.Range(55.0f,212.0f);
		c0.r = r;
		c0.g = g;
		c0.b = b;
    ...
    void Update()
    {
		...
		if (count != 0) {
			...
			for (int i = count - 1; i >= 0; i--) {
				...
				Shaderding(metaballs[i].transform, c[i]);
    ...
    void Shaderding(Transform mb,Color col)
    {
        //needs changing
        //mat.SetFloat("_Shininess", (Mathf.Sin(Time.time*2)+1)*0.14F+0.03F);
		float outline = (Mathf.Sin(Time.time*2)+1)*0.003F+0.003F;
